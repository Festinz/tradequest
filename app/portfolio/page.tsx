...
export default async function PortfolioPage() {
  ...
  const { data: positions } = await supabase
    .from("positions")
    .select("ticker, qty, avg_price, stocks(name, market)")
    .eq("user_id", user.id);

  const { data: trades } = await supabase
    .from("trades")
    .select("id, ticker, side, qty, price, status, pnl, pnl_pct, opened_at, closed_at, stocks(name)")
    .eq("user_id", user.id)
  ...
  const winRate =
    closed.length > 0
      ? Math.round((closed.filter((t) => Number(t.pnl_pct) > 0).length / closed.length) * 100)
      : 0;
  ...
  return (
    <AppShell active="/portfolio">
      ...
      <section className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryCard label="총 자산" value={formatKRW(total)} accent={pnlColor(totalPnlPct)} sub={formatPct(totalPnlPct)} />
        <SummaryCard label="예수금" value={formatKRW(character.cash_balance)} />
        <SummaryCard label="주식 평가" value={formatKRW(positionsValue)} sub={`${(positions ?? []).length}종목`} />
        <SummaryCard label="승률 (청산 기준)" value={`${winRate}%`} sub={`${closed.length}건`} />
      </section>
        ...
        <section className="rounded-card border border-border bg-surface-0 p-5">
          ...
          {positions && positions.length > 0 ? (
            <ul className="space-y-2">
              {positions.map((p) => {
                const stock = (p as unknown as { stocks: { name: string; market: string } | null }).stocks;
                return (
                  <li key={p.ticker}>
                    <Link
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="truncate text-caption text-text-3">
                          {p.ticker} · {p.qty}주 · 평단 {formatKRW(p.avg_price)}
                        </p>
                      </div>
          ...
          {trades && trades.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-body">
                ...
                <tbody>
                  {trades.map((t) => {
                    const stock = (t as unknown as { stocks: { name: string } | null }).stocks;
                    return (
                      <tr key={t.id} className="border-b border-border last:border-b-0">
                        <td className="px-3 py-2 text-caption text-text-3">{formatDateKR(t.opened_at)}</td>
                        <td className="px-3 py-2">
                          <Link href={`/trade?symbol=KRX:${t.ticker}`} className="hover:text-brand">
                            {stock?.name ?? t.ticker}
                          </Link>
                          <span className={`ml-2 text-caption ${t.side === "buy" ? "text-bullish" : "text-bearish"}`}>
                            {t.side === "buy" ? "매수" : "매도"}
                          </span>
                        </td>
                        <td className="px-3 py-2 text-right font-mono">{t.qty}</td>
                        <td className="px-3 py-2 text-right font-mono">{formatKRW(t.price)}</td>
                        <td className="px-3 py-2 text-right">
                          {t.status === "closed" && t.pnl_pct !== null ? (
                            <span className={`font-mono ${pnlColor(Number(t.pnl_pct))}`}>
                              {formatPct(Number(t.pnl_pct))}
                            </span>
                          ) : (
                            <span className="text-caption text-text-3">보유 중</span>
                          )}
                        </td>
                      </tr>
                    );
            ...
            <p className="py-6 text-center text-caption text-text-2">매매 기록이 없어.</p>
          )}
        </section>
      </div>
    </AppShell>
...
function SummaryCard({
  ...
  return (
    <div className="rounded-card border border-border bg-surface-0 p-5 shadow-card">
      ...
      {sub && <p className={`text-caption font-mono ${accent ?? "text-text-3"}`}>{sub}</p>}
    </div>
  );
}
